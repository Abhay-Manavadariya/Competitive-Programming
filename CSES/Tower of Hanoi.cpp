...
#define ll                      long long
#define TESTCASE                int t = 1; cin >> t; while(t--) func();
...
#define floop(i_itr, k, n)       for(int i_itr = k; i_itr <= n; i_itr++)
#define flooprev(i_itr,n,a)      for(ll i_itr=n; i_itr>=a; i_itr--)
...
ll sqrt_bin(ll x) { ll left = 0, right = 2000000123; while (right > left) { ll mid = (left + right) / 2; if (mid * mid > x) right = mid;else left = mid + 1;}return left - 1;}
...
int sum_series(int a, int l, int n) {int res = 0;if(n % 2 == 0) {res = ((a + l) * (n / 2));}else res = ((a + l)/2) * n;return res;}
...
void rec(int n,int s,int h,int d)
{
    if(n==1)
    {
        cout<<s<<" "<<d<<endl;
    ...
    rec(n-1,s,d,h); //first n-1 plates moves to source to helper
    cout<<s<<" "<<d<<endl; // now move 1 plate source to destination
    rec(n-1,h,s,d); // now, move the remaining plate that are in helper, move to destination using the source. now source acts as a helper and helper acts as a source.
}
